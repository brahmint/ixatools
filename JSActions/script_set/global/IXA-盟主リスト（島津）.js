...
function jsa_IXA_SoldiersCount(){
	...
	var urltop = 'http://' + window.location.host + '/alliance/list.php?c=10&p=';
...
{
	var aurl = alliance[num].leadersprof;
	//alert("url="+url);
	var req = new XMLHttpRequest();
	...
	req.onreadystatechange = function (aEvt) {
		if (req.readyState == 4) {
			if(req.status == 200) {
				...
				if (!tbl) {
					alert("num="+num);
					alliance[num].palacename = "";
					alliance[num].palacepos  = "";
					alliance[num].palacepopu = "";
				} else {
					var tds = getTags(tbl.join(""), "td");
					//
					//alert("trs.length="+trs.length+"\n" + "trs="+trs.join(","));
