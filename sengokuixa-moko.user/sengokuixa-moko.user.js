...
// @description	戦国IXA用ツール ver 1.8.6a 20110330 + 婆羅門機能追加 20110901
...
//		$('INPUT[name="dungeon_select"][value="'+options['hikyou_where']+'"]').attr('checked', true);
	...
	function all_dissolution() {
		...
		if(location.pathname != "/card/deck.php") return;
		/*
		var tmp = '<li><a href="javascript:void(0);" onclick="return false;" id="deck_dissolution"><img src="http://www.jj-midi.com/image/btn_all_breakup.png" alt="全部隊解散" style="position: relative; top: -4px; "></a></li>';
		*/
	...
	function deck_dissolution($tmp) {
		var work_id='';
		var p='';
		var w_null='';
		if($tmp==undefined) {
			if($('img[alt^="解散"]').attr('alt')==undefined) {
				location.href='/card/deck.php';
				return;
			}
			work_id = $('img[alt^="解散"]').parent().attr('onclick').toString().split(',');
			p = $('#p').attr('value');
		} else {
			if($tmp.find('img[alt^="解散"]').attr('alt')==undefined) {
				location.href='/card/deck.php';
				return;
			}
			work_id = $tmp.find('img[alt^="解散"]').parent().attr('onclick').toString().split(',');
			p = $tmp.find('#p').attr('value');
		}
		var unit_assign_id = work_id[0];
		unit_assign_id=unit_assign_id.replace("function onclick(event) {\n",'');
		unit_assign_id=unit_assign_id.replace('  confirmUnregist(','');
		unit_assign_id=unit_assign_id.replace(/ /g,'');
		unit_assign_id=unit_assign_id.replace(/\'|\"/g,'');
		var unset_unit_squad_id=work_id[1].replace(/\'|\"/g,'');
		var data = {select_assign_no:0,unit_assign_id:unit_assign_id,unset_unit_squad_id:unset_unit_squad_id,change_unit_squad_id:w_null,dungeon_unit_squad_id:w_null,dungeon_card_id:w_null,dungeon_select:w_null,deck_mode:'nomal',p:p};
		$.ajax({
			type: "POST",
			url: '/card/deck.php',
			data:data,
